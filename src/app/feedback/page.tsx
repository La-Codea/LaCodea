...
// ✅ SimpleTime Feedback (Form)
import SimpleTimeFeedback from "@/sites/simpletime/pages/feedback";

function getAppFeedbackHref(slug: string, locale: string) {
  const prefix = locale === "en" ? "" : `/${locale}`;

  if (process.env.NODE_ENV !== "production") {
    return `http://${slug}.localhost:3000${prefix}/feedback`;
  }

  const domain = process.env.ROOT_DOMAIN ?? "lacodea.com";
  return `https://${slug}.${domain}${prefix}/feedback`;
}

export default async function FeedbackPage() {
  const site = await resolveSite();

  // ✅ Subdomain simpletime => direkt zur SimpleTime Feedback Seite
  if (site.key === "simpletime") {
    return <SimpleTimeFeedback />;
  }
  ...
  return (
    <main className="container py-12">
      ...
      <p className="muted mt-2">{t("pages.feedback.chooseApp")}</p>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {apps.map((app) => {
          ...
          return (
            <a
              key={app._id}
              href={getAppFeedbackHref(app.slug, locale)}
            ...
            >
              ...
              {desc ? <p className="muted mt-3 text-sm leading-relaxed">{desc}</p> : null}

              <div className="mt-4">
                <span className="btn btn-primary">{t("pages.feedback.openFeedback")}</span>
              </div>
            </a>
          );
        })}
